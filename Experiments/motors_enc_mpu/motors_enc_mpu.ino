...
void printRobotData() {
  ...
  // Print encoder data + essential MPU data
  Serial.print("E,");
  Serial.print(left);
  Serial.print(",");
  Serial.print(right);
  ...
  Serial.print(",");
  Serial.print(rightDirection ? "F" : "R");
  Serial.print(",YAW,");
...
void setup() {
  ...
  Serial.println("  P                - Print robot data once");
  ...
  Serial.println("  E                - Toggle data printing (on by default)");
  Serial.println("Format: E,leftPulses,rightPulses,leftPWM,rightPWM,leftDir,rightDir,YAW,angleZ,GYROZ,gyroZ");
}

void loop() {
  // Print encoder and essential MPU data continuously
  ...
  if (Serial.available()) {
    ...
    if (cmd.length() > 0) {
      ...
      // Print robot data once
      else if (command == 'P' || command == 'p') {
        printRobotData();
      }
      // Toggle data printing
      else if (command == 'E' || command == 'e') {
        // Print is always on now, this just shows current state
        Serial.println("Robot data printing is always on");
        printRobotData();
      }
      else {
