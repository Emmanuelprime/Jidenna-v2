...
void initMPU6050() {
  Wire.begin(I2C_SDA, I2C_SCL);  // Use custom I2C pins
  mpu6050.begin();
...
void setLeftPWM(int pwm) {
  ...
  ledcWrite(0, leftCurrentPWM);  // Use LEDC for ESP32
}

void setRightPWM(int pwm) {
  rightCurrentPWM = constrain(abs(pwm), 0, MAX_PWM);
  ledcWrite(1, rightCurrentPWM);  // Use LEDC for ESP32
}
...
void setup() {
  ...
  // Configure PWM for ESP32 using LEDC
